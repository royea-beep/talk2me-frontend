...
export default async function Page() {
  const data = await fetchTable("gprompt_registry", { limit: 200 });
  return (
    <div>
      <h2 className="text-2xl font-semibold text-slate-100 mb-1">G-PROMPTs</h2>
      <p className="text-sm text-slate-400 mb-6">{data.total_count} rows - click row to expand</p>
      {data.rows.length === 0 ? (
        <div className="rounded border border-slate-800 bg-slate-950/60 p-6 text-sm text-slate-400">No G-PROMPTs found.</div>
      ) : (
